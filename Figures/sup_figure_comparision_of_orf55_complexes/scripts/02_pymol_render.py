...
Run:  FIG_ROOT=$PWD pymol -cq scripts/02_pymol_render.py   (PyMOL 2.5+/3.x; needs models/ from 01_select_models.py)
Chains: A=ORF55, B=ATP, C=continuous strand, D=upstream arm (3'-OH), E=downstream arm (5'-P)."""
import os
from pymol import cmd

# PyMOL sets __file__ to its own path, so the project root comes from $FIG_ROOT (set by run_all.sh) or the cwd
ROOT = os.environ.get("FIG_ROOT", os.getcwd())
M   = os.path.join(ROOT, "models")
OUT = os.path.join(ROOT, "output", "parts")
